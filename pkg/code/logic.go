...
func extractSwitchBlocks(lines []string, lang string, isNew bool) []SwitchSignature {
	var blocks []SwitchSignature
	...
	for i := 0; i < len(lines); i++ {
		...
		if (isNew && strings.HasPrefix(line, "+")) || (!isNew && strings.HasPrefix(line, "-")) {
			...
			if m := switchRegex.FindStringSubmatch(l); m != nil {
				...
				blocks = append(blocks, SwitchSignature{Expr: expr, Cases: cases})
			}
		}
	}
...
func FormattedLogic(line, lang, filename string) string {
	...
	if len(oldSwitches) > 0 && len(newSwitches) == 0 {
		makeResult := func(switches []SwitchSignature) string {
	...
	if len(oldSwitches) > 0 && len(newSwitches) > 0 {
		...
		makeResult := func(osw, nsw SwitchSignature) string {
			var b strings.Builder
