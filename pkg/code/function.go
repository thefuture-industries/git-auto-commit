...
func ParseToStructureFunction(line, lang string) *FunctionSignature {
	switch lang {
	case "go":
...
func FormattedFunction(diff, lang string) string {
	...
	var oldFunc, newFunc *FunctionSignature
...
func parseGoFunction(line string) *FunctionSignature {
	...
	for _, p := range strings.Split(paramsString, ",") {
		...
		if len(parts) == 2 {
			params = append(params, FunctionParameters{Name: parts[0], Type: parts[1]})
		}
	}

	return &FunctionSignature{Name: name, Params: params}
}

func parsePythonFunction(line string) *FunctionSignature {
	...
	for _, p := range strings.Split(m[2], ",") {
		...
		if len(parts) == 2 {
			params = append(params, FunctionParameters{Name: strings.TrimSpace(parts[0]), Type: strings.TrimSpace(parts[1])})
		} else {
			params = append(params, FunctionParameters{Name: p, Type: ""})
		}
	}
...
func parseTSJSFunction(line string) *FunctionSignature {
	...
	if m != nil {
		...
		return &FunctionSignature{Name: name, Params: params, ReturnType: ""}
	}

	return nil
}

func parseCJavaFunction(line string) *FunctionSignature {
	...
	for _, p := range strings.Split(m[3], ",") {
		...
		parts := strings.Fields(p)
		if len(parts) == 2 {
			params = append(params, FunctionParameters{Name: parts[1], Type: parts[0]})
		} else if len(parts) == 1 {
			params = append(params, FunctionParameters{Name: parts[0], Type: ""})
		}
	}
...
func parseCSharpFunction(line string) *FunctionSignature {
	...
	return &FunctionSignature{Name: name, Params: params, ReturnType: m[3]}
}
