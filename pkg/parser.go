		...
		go func() {
			...
			for file := range jobs {
				...
				for _, formatted := range []string{
					...
					code.FormattedClass(diff, lang),
					code.FormattedLogic(diff, lang, filepath.Base(file)),
					code.FormattedImport(diff, lang, filepath.Base(file)),
					code.FormattedStruct(diff, lang),
					code.FormattedType(diff, lang),
					code.FormattedInterface(diff, lang),
					code.FormattedEnum(diff, lang),
				} {
					if formatted != "" {
